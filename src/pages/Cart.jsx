...
const Cart = ({ cart, setCart }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 p-6 text-white min-h-screen">
      ...
      <div className="w-full md:w-[55%]">
        {cart.length !== 0 ? (
          cart.map((item) => (
            <div
              key={item.id}
              className="flex flex-col md:flex-row justify-center items-center gap-4 p-4 bg-zinc-800 rounded-md my-2"
            >
              <img
                src={item.images[0]}
                alt={item.title}
      ...
      <div className="w-full md:w-[30%] bg-zinc-900 p-6 rounded-xl h-fit">
